package com.fomopay.demo.linechart
...
import com.fomopay.demo.R
import com.fomopay.demo.linechart.animation.LineChartAnimator
import com.fomopay.demo.linechart.formatter.DefaultYAxisValueFormatter
import com.fomopay.demo.linechart.formatter.YAxisValueFormatter
import com.fomopay.demo.linechart.model.Label
import com.fomopay.demo.linechart.touch.OnScrubListener
import com.fomopay.demo.linechart.touch.ScrubGestureDetector
import com.fomopay.demo.linechart.touch.ScrubGestureDetector.ScrubListener
import com.fomopay.demo.linechart.utils.ScaleHelper
import com.fomopay.demo.linechart.utils.Utils.getBitmapFromVectorDrawable
import com.fomopay.demo.linechart.utils.Utils.getNearestIndex
...
class LineChartView @JvmOverloads constructor(
    ...
    private var scrubGestureDetector = ScrubGestureDetector(
        ...
        ViewConfiguration.get(context).scaledTouchSlop.toFloat(), WeakReference(this)
    )
    ...
    private fun processAttributes(
        ...
        try {
            ...
            fillType = a.getInt(
                R.styleable.LineChartView_linechart_fillType,
            ...
            labelBackgroundColor =
                a.getColor(R.styleable.LineChartView_linechart_labelBackgroundColor, 0)
            labelBackgroundRadius =
                a.getDimension(R.styleable.LineChartView_linechart_labelBackgroundRadius, 0f)
            labelBackgroundPaddingHorizontal = a.getDimension(
                R.styleable.LineChartView_linechart_labelBackgroundPaddingHorizontal,
                0f
            )
            labelBackgroundPaddingVertical = a.getDimension(
                R.styleable.LineChartView_linechart_labelBackgroundPaddingVertical,
                0f
            )
            zeroLabelTextColor =
                a.getColor(R.styleable.LineChartView_linechart_zeroLabelTextColor, 0)
            zeroLabelTextSize =
                a.getDimension(R.styleable.LineChartView_linechart_zeroLabelTextSize, 0f)
            zeroLabelBackgroundColor =
                a.getColor(R.styleable.LineChartView_linechart_zeroLabelBackgroundColor, 0)
            ...
            scrubCursorDrawable =
                a.getDrawable(R.styleable.LineChartView_linechart_scrubCursorDrawable)
            ...
            isZeroLineEnabled =
                a.getBoolean(R.styleable.LineChartView_linechart_zeroLineEnabled, true)
    ...
    private fun configEditMode() {
        ...
        adapter = object : LineChartAdapter() {
            private val yData = floatArrayOf(
                68f,
                22f,
                31f,
                57f,
    ...
    private fun populateLabels() {
        ...
        for (i in 1 until gridLinesY.size) { // No label in the last grid line
            // Format text
            labelText = yAxisValueFormatter.getFormattedValue(
        ...
        if (shouldDrawZeroLine()) {
            labelText =
                yAxisValueFormatter.getFormattedValue(0f, adapter.getDataBounds(), gridYDivisions)
            zeroLabelTextPaint.getTextBounds(labelText, 0, labelText.length, labelTextBounds)
            bgTop =
                scaleHelper.getY(0f) - labelTextBounds.height() / 2f - labelBackgroundPaddingVertical
    ...
    private fun populateScrubCursor(newScrubCursorTargetPos: PointF) {
        when {
            ...
            newScrubCursorTargetPos == scrubCursorTargetPos -> return
                ...
                scrubCursorTargetPos = newScrubCursorTargetPos
                val animX = PropertyValuesHolder.ofFloat(
                    "x",
                    scrubCursorCurrentPos!!.x,
                    scrubCursorTargetPos!!.x
                )
                val animY = PropertyValuesHolder.ofFloat(
                    "y",
                    scrubCursorCurrentPos!!.y,
                    scrubCursorTargetPos!!.y
                )
                scrubAnimator.setValues(animX, animY)
                scrubAnimator.duration = 50
                scrubAnimator.addUpdateListener { animation: ValueAnimator ->
                    scrubCursorCurrentPos = PointF(
                        animation.getAnimatedValue("x") as Float,
                        animation.getAnimatedValue("y") as Float
                    )
                    invalidate()
                }
                scrubAnimator.start()
